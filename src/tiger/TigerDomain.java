...
public class TigerDomain implements DomainGenerator {
    ...
    /**
     * The observation attribute value for hearing the tiger behind the
     * left door.
     */
    ...
    /**
     * The observation value for when reaching a new pair of doors (occurs
    ...
    /**
     * Returns a {@link burlap.mdp.auxiliary.StateGenerator} that 50% of the time
     * generates an hidden tiger state with the tiger on the
    ...
    /**
     * Returns a {@link burlap.mdp.auxiliary.StateGenerator} that some of the
     * of the time generates an hidden tiger state with the tiger on the
     * left side, and others on the right. Probability of left side is
     * specified with the argument probLeft
     * @param probLeft the probability that a state with the tiger on the
    ...
    /**
     * Generates an initial {@link TabularBeliefState} in which the it is equally
     ...
     * By default, the TerminalExplorer interacts with the partially observable
     * environment ({@link burlap.mdp.singleagent.pomdp.SimulatedPOEnvironment}),
     * which means you only get to see the observations that the agent would.
     * However, if you set the first command-line argument
     * to be "h", then the explorer will explorer the underlying fully
     * observable MDP states.
